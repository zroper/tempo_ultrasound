...
process WINDOWS(int curr_target,						// see SETC_TRL
	...
	oMove(object_targwin, stim_ecc_x*deg2pix_X , stim_ecc_y*deg2pix_Y); //move the animated graph window to location
	
	// calculate the params of the target window
	half_size = targ_win_size/2;
	Targ_win_left	= stim_ecc_x - half_size;
	Targ_win_down	= stim_ecc_y + half_size;
	Targ_win_right	= stim_ecc_x + half_size;
	Targ_win_up		= stim_ecc_y - half_size;
