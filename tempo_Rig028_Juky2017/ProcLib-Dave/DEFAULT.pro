...
process DEFAULT(int state, 
	...
	declare hide int euler		= 6; // in this case e is 6 not 2.71828183
	declare hide int broca		= 7;	
	declare hide int helmholtz	= 8;
	declare hide int gauss		= 9;
	...
	if(monkey == euler)
		...
		if (state == run_delayed_sess)
			...
			Min_Holdtime			= 400;  // minimum time after fixation before target presentation
			Max_Holdtime			= 400; // maximum time after fixation before target presentation
	...
	if(monkey == upton)
		...
		if(room == 28)
			{
			Subj_dist	= 490.0; 	
			TrainingStill = 1;	// Indicates that we are using motion detector to train the monk to be still
			}
		else if (room == 29)
			{
			Subj_dist	= 489.0;
...
///mem guided	
	...
	if (state == run_fix_sess)
		...
		if (state == run_delayed_sess)
			...
			Min_Holdtime			= 200;  // minimum time after fixation before target presentation
			Max_Holdtime			= 200; // maximum time after fixation before target presentation
...
/* 			Angle_list[0]			= 0;	// angle of each target individually (degrees)
		...
		//--------------------------------------------------------------------------------------------------------------------
	// Fixation task (this goes last b/c these settings trump any other ones if the animal is doing calibration)
