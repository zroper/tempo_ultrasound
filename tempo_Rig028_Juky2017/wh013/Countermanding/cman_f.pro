...
declare DEBUG;
declare AUTOMONKEY;
declare DEVELOPMENT = 1;

// Development system does not have Plexon
declare PLEXON = !DEVELOPMENT;
...
process STIM()
{
	...
	if (Tpos_0_bias==1)
	{
		printf("Stop Fraction biais toward a specific target location \n");
		rand_stop_go=1 + random(100);
	...
	*/
	...
	else if (types[j] == 2)
	{
		...
		if(Stimu[s]==2)
		{
			printf("Zap NOGo Trial \n");
			printf("SSD=%dms\n",stop[s]);
		}
		if(Stimu[s]==1)
		{
			printf("ISNOTNOGo Trial \n");
		}
	}
	else if (types[j] == 0)
	{
	...
	if(!failed)
	{
		...
		if(types[j] == 1 || Stimu[s]==1)										 // If a no-stop trial...
		{
			...
			else if(!In_Target)					 // Else if they didn't saccade to target on time...
			{
				...
				LastTrialOutcome=1;
				printf("Failed to saccade to target on time!\n");			
				correctmarker=0;
