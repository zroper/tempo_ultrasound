...
/* 
   ** TEMPO DOS IMPROVEMENT REPORT **
   ...
   - trail balancing passing during the catch trials
...
/* initialization part */
 ...
 declare DRAW_STIM();  /* sub of trial                 */     
...
process MAIN enabled
{
	...
	//eall2 also draws photosensor mark on all windows      
	dsend("eall2");             // frame 0: erase all           						
	dsend("z = 0;");     	    // init z variable              
	nexttick;
...
process TRIAL_CONF
{
	...
	*/
	...
	/* target replacement in the target position */
	if ( tIsCatch == 0 ){
...
process DRAW_STIM
{
   ...
   //	printf(" COLOR CODE    %d\n",iColor);
	...
	/* filled fixation */
	dsendf("nprepSrch(%d,%d,%d,%d,",1,iFFix,eFP_r,eFP_a);
	dsendf("%d,%d,%d,%d);",iFixS,iColCopy[iFFix],mHolder,iMask[lPair]);
	nexttick;

	/* frame 2: fixation: fixation property 0 is fixed: dummy   */
	/* empty fixation */
	dsendf("nprepSrch(%d,%d,%d,%d,",2,iEFix,eFP_r,eFP_a);
	dsendf("%d,%d,%d,%d);",iFixS,iColCopy[iEFix],mHolder,iMask[lPair]);
	nexttick;
	
	
	/* frame 3: search + go fixation + without photodiode input */
	i = 0;
	dsendf("nprepSrch(%d,%d,%d,%d,",3,iEFix,eFP_r,eFP_a);
	dsendf("%d,%d,%d,%d);",iFixS,iColCopy[iEFix],mHolder,iMask[lPair]);
	while( i < mMAXPOS ){
		dsendf("nprepSrch(%d,%d,%d,%d,",3,lObjt[i],145*175*mEccy/1000,i);
		dsendf("%d,%d,%d,%d);",eCIRsize,iColCopy[lObjt[i]],mHolder,iMask[lPair]);
		i = i + 1;
		nexttick;
	}
	...
	*/
	...
	i = 0;
	dsendf("nprepSrch(%d,%d,%d,%d,",4,iEFix,eFP_r,eFP_a);
	dsendf("%d,%d,%d,%d);",iFixS,iColCopy[iEFix],mHolder,iMask[lPair]);
	while( i < mMAXPOS ){
		dsendf("nprepSrch(%d,%d,%d,%d,",4,lObjt[i],145*175*mEccy/1000,i);
		dsendf("%d,%d,%d,%d);",eCIRsize,iColCopy[lObjt[i]],mHolder,iMask[lPair]);
		i = i + 1;
		nexttick;
	}
	...
	i = 0;
	dsendf("nprepSrch(%d,%d,%d,%d,",9,iFFix,eFP_r,eFP_a);
	dsendf("%d,%d,%d,%d);",iFixS,iColCopy[iFFix],mHolder,iMask[lPair]);
	while( i < mMAXPOS ){
		dsendf("nprepSrch(%d,%d,%d,%d,",9,lObjt[i],145*175*mEccy/1000,i);
		dsendf("%d,%d,%d,%d);",eCIRsize,iColCopy[lObjt[i]],mHolder,iMask[lPair]);
		i = i + 1;
		nexttick;
	}
	...
	/* frame 8: reappear                         */
	if ( tIsCatch == 0 ){
		dsendf("nprepSrch(%d,%d,%d,%d,",8,iEFix,eFP_r,eFP_a);
		dsendf("%d,%d,%d,%d);",iFixS,iColCopy[iEFix],mHolder,iMask[lPair]);
		dsendf("nprepSrch(%d,%d,%d,%d,",8,lObjt[lTgtP],145*175*mEccy/1000,lTgtP);
		dsendf("%d,%d,%d,%d);",eCIRsize,iColCopy[lObjt[lTgtP]],mHolder,iMask[lPair]); nexttick;
	}

	/* dummy string for complete transfer check up */
   	dsend("disp z");                         
 	while (com_get() != 'z')  nexttick;	      														 
}
...
process IDLE
{
	...
	dsend("f5;");	/* screen f5 will be blank by virtue of eall2. */
...
process TRIAL
{
	...
	while( tGoOn ){
		...
		//true and eye is in fixation window)
		   	tDead = time() + tFixAcqT; 
		...
		while( time() < tDead ){
  			 // STIMULATE: FOR ALL TASKS WITH uStim DELAY < 0
  			 if (  (didStim == 0) && (trlStimP <= mStimProb) && (mStimDelay <= 0) && (time() >= mStimDelay + tDead) ){ /* last argument counts backwards from time 0, where time 0 is the point at which fixation parameters have been met (e.g., have held for 1000 ms). */
  				spawn mSTIM;
				didStim = 1;
			 }
		...
		if ( lTask ){	 // if 0, will skip next part and run only fixation.  If 1, 2, will show some type of search array, detection, MG, or search
		...
		//remained in fixation area for designated amount of time (maxTgtW)
				...
				if ( tIsCatch ){
					...
					while( time() < tDead ){
			   		...
			   		// STIMULATE: FOR CATCH TRIALS WITH DELAY >= 0
			 		if (  (didStim == 0) && (trlStimP <= mStimProb) && (mStimDelay >= 0) && (time() >= mStimDelay + StartTime) ){
   					spawn mSTIM;
				 	didStim = 1;
				 	}
					...
					tDead = time() + tHt;
	  				...
	  				while( time() < tDead ){ //condition will fail if tHt == 0 and won't run
		   				...
		   				// STIMULATION: FOR MEMORY GUIDED TASK WITH DELAY >= 0
				  		StartTime = time();
				  		if (  (didStim == 0) && (trlStimP <= mStimProb) && (mStimDelay >= 0) && (time() >= mStimDelay + StartTime) ){
				   			spawn mSTIM;
			 	   			didStim = 1;
					...
					//tDead = time() + tMaxSacL;
					// instead of above, set array time to SAT cutoff times
					if (SAT_cond == 1) tDead = time() + SAT_slow;
					if (SAT_cond == 2) tDead = time() + SAT_med;
					if (SAT_cond == 3) tDead = time() + SAT_fast;
					
					
					while( time() < tDead && eInFix ) {
				 		// STIMULATION: FOR NON-CATCH TRIAL SEARCH & DETECTION W/ DELAY >= 0
						if (  (didStim == 0) && (trlStimP <= mStimProb) && (mStimDelay >= 0) && (time() >= mStimDelay + StartTime) ){
   				   			spawn mSTIM;
				   			didStim = 1;
				 		}
...
process SHOW_STIM
{
	...
	//tArray sets the amount of time to show the stimuli.  For MG, hard coded to 100 ms
	...
	if ( tHt == 0 ){
			...
			if ( tArray - tHt > 0 ){
				...
				dsend("f1;"); nexttick;	 // filled fixation
				wait(tHt - tArray); nexttick;
				
				//event_set(1,0,EV_HOLD_OFF); nexttick;
				
				dsend("f2;"); nexttick;	// empty fixation
				//spawn MARKER2(EV_HOLD_OFF); nexttick;
				//waitforprocess MARKER; nexttick;
...
process UPDATE
{
  ...
  //		nexttick;
   ...
   //		} else {
	...
	if (SAT_cond == 1) eventCode = rwd_slow;
