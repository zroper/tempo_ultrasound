...
declare int nTrials = 150;		// Number of different trials in a cycle.
...
declare frac_no_reward = 50;                                // % of no reward trial (which should be rewarded
...
declare REWARD_SIZE=100;				 // Amount of time to give juice.
declare REWARD_MULTIPLIER =1;
...
declare fix_t= 700; 				                // Presentation time of fixation point.
declare fix_jitter= 20;
declare hold_fixation_time;
declare stop_fix_t = SSD_max*2;	 		// Minimum time to fixate center after stop signal is presented.
...
process STIM()
{
	...
	if(EXPO_JITTER==1)
	{
		...
		float_frac_hold_jitter=(frac_hold_jitter/100.0)*fix_t*expo_2*2.0;
...
process REWARD()
{
	...
	if ( temp_rand_mult >= frac_mult_reward && types[j]==2)
	{	
	...
	if (temp_rand_mult >= frac_mult_reward)
	{	
		wait(realRewardSize);
	}
	else
