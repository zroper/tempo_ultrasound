...
declare end_init_fixation_time;
declare max_init_fixation_time = 5000;
...
declare int nTrials = 100;		// Number of different trials in a cycle.
...
declare int NOGO_RATIO =35;	 // Fraction of nogo trial
...
declare SSD_Step=50;									// SOA steps
declare SSD_min=100;										// SOA min
declare SSD_max=300;  									// SOA max
...
declare staircaseSSD1 = SSD_min;			// MODIFIED 1-15-2008 EEE  ADDED staircaseSSD to increase/decrease the SSD for on the current trial
...
declare REWARD_RATIO = 95;				// Fraction of rewarded trial ( 09/21/2004 =1 Loop need to be added
declare frac_no_reward = 5;                                // % of no reward trial (which should be rewarded
declare frac_extra_reward =5;
declare feedback = 1;                                             //  flag whether to give feedback about errorby turning off the target
declare Bell=1;                                                   	// postive 2ndary reinforcer
declare REWARD_SIZE=70;				 // Amount of time to give juice.
declare SOUND_REWARD=0;                              	// Acoustic reinforcement
declare Block=0;
declare tot_others;
declare ind_others;
declare neg_2reinforcement=1;				// negative second reinforcer  = no sound  !!! Note: that this only occurs on noncancelled trials when this flag ==1
declare PAUSE_REWARD=400;				// delay between correct or incorrect and juice
/////////////////////////////////////////////////////
declare FIXED_JITTER=1;					// JITTER is Linear (random(value) + or -)
declare EXPO_JITTER=0;					// JITTER is "expo" number (right part of normal distribution + constant
///////////////////////////// FRACTION JITTER IF LINEAR
declare int frac_fix_jitter=20;	 			// fixation
...
declare inter_trial = 1000;  					// Intertrial interval variables.
...
declare EOT_delay=200;                     // delay after end of trial to have room for psacdet. modified 051606
...
declare col_tar = 10;											// VideoSync variable for the color of the target.
declare Lum=50;
...
declare STIM_OFFSET=0;					//MWL 6/12/06 changed from 52 to 0
declare STOPZAP=0;
declare STIM_FLAG=0;
...
{
	spawn ANIMATION;
	time_stop_signal = time()  + 30000000; // did this because the stim was being presented with the target EEE
	//////////FIXATION ON//////////	
	...
	end_init_fixation_time = time() + max_init_fixation_time;
	
	while (time()<end_init_fixation_time && !In_Fixation_Window)
		nexttick;
	...
	if (time() > end_init_fixation_time)
	{
		spawn END_TRIAL;
	}
	...
	if(types[j] == 2)
	{
			...
			spawn SendTTLToRemoteSystem(2653);
				...
				if(SOUND_S==0)// if not auditory stop signal
				{					
					while ( time() < time_stop_signal - Vrefresh) //This MAKEs IT SO THAT IF EARLY NONCANCELLED STOP SIGNAL IS NEVER PRESENTED
						nexttick;
					
					if (failed == 0) //present STOP SIGNAL (fix spot) but only if didn't leave fix early
					{
...
process TRIAL_LOOP()
{
	...
	while (1)                           						 // Loop Ntrials * NBlocks
	{
		...
		// Beginning of the Trial (TrialStart_)
...
process WRONG()
{
	...
	// add pierre 01/10/06
	//suspend WATCH_TIME;	  // suspend Watch time
	//suspend DRAW_STIM;   //suspend draw stimuli on screen
	//dsend("cl");
	//printf("         \n");
	//printf("         \n");
	//printf("         \n");
...
process END_TRIAL()
{
	...
	suspend MARKER_FIX;	   // added 051606 to close the photodiode loop if the Stop signal is not presented
	...
	if (i>nTrials-1)
	{
		...
		if (STOP_FRACTION_STEP_FLAG)
		{
			...
			{
				NOGO_RATIO_Dial = 50;
				print ("Switch to 50% stop");
			}
			else if (NOGO_RATIO == 50)
			{
				NOGO_RATIO_Dial = 20;
				print ("Switch to 20% stop");
			}
		}
	}
}
