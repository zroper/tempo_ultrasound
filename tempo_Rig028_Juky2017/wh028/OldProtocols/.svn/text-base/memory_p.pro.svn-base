...
declare Fix_H = 200;	   						 // Size of box in which eye can be and be said to fixate center.
declare Fix_V = 200;
...
declare Targ_H =350;                          // Size of box in which eye can be and be said to fixate target.
declare Targ_V = 300;
...
declare memory_fix_t = 500;										 // Time to fixate center after stop signal is presented.
declare frac_stop_t_jitter=20;
declare memory_fix_t_jitter;
declare memory_fix_time;
declare fix_targ_maxT =  2400; //memory_fix_t*3 ; MWL							 // Maximum RT for moving from fixation window to target window.
declare Sacc_time=300;													   // Time between leaving fixation and enter inot the Target
declare step_lat;
declare trial_start;
declare step_wind;
declare step_targ;
declare Hold_on_target=400;
declare Hold_targ;
...
declare intertrial, inter_trial = 1000;							 // Intertrial interval variables.
...
declare tx=0;
declare nTrials;
								 // Number of Trials in a cycle. We choose 40 because it is the smallest number that is divisible w/o remainder by 4 and 10.  
//10 is the number of different stop signal times. We need also a loop because we want 25% 33% 50% of the trials to be stop. 
...
process INITIALIZE()
{
	...
	// These are the VideoSync sprites.
    dsendf("oc 1,3\n");                          				 // Cross hair for "eye"
    dsendf("ow 1,20,20\n");                      				 // width & height
    dsendf("oi 1,13\n");                         				 // color
    //dsendf("os 1\n");                            				 // make visible
           ...
           if(failed==0 && step_lat==0)
			  {
                ...
                dsendf("co col_tar;");	 //turn target back on
...
TarV = -sin(ang+tpos+ang_2+ang_3+ang_4)*eccen;  
}
...
{
TarH = cos(ang+tpos+ang_2+ang_3+ang_4 - 180)*eccen;	  //// oposite to 2 
TarV = -sin(ang+tpos+ang_2+ang_3+ang_4 - 180)*eccen;
}
...
///////////////////////////////////////////////////////////
    ...
    dsendf("co 10;"); //MWL changed 15 to 10											 // 15 = white.
	dsend("xm fix(0, 0, 10)");	  //MWL changed 15 to 10									  
...
//5 Empty
 ...
 eventCode=2735;			   
 spawn queueEvent();	
 eventCode=frac_stop_t_jitter;			   
 spawn queueEvent();	

 // 16 Length of target presentation MWL
 eventCode=2736;			   
 spawn queueEvent();	
 eventCode=Stimulus_Time;			   
 spawn queueEvent();
...
/*                                                                    */
/* //////////////// Lower left corner H coordinates                   */
/*  eventCode=abs(-Fix_H/2);			                              */
/*  spawn queueEvent();                                               */
/*  eventCode=0;			                                          */
/*  spawn queueEvent();                                               */
/*                                                                    */
/* //////////////// Lower left corner V coordinates                   */
/*  eventCode=abs(-Fix_V/2);			                              */
/*  spawn queueEvent();                                               */
/*  eventCode=0;			                                          */
/*  spawn queueEvent();                                               */
/*                                                                    */
...
process MARKER()
{
	...
	while (!MarkerOn)                   						 // Keep going provided we haven't see the pulse...
    {
	    ...
	    while (index < nAsets)               					 // index=0,1,2,3 (scan through ASETS samples) ..
        {
        	...
        	if (ctable_set(2,index,0))          				 // If (Counter5[index] != 0) ...
            {       
...
process INIT_FIX_CORRECTOR()
{
	...
	while(in_init_fixation_window == 0 && In_Fixation_Window==0)  /// && In_Fixation_Window==0	added 10/07/2005
	{
		...
		dsendf("co 10;"); //MWL changed from 15 to 10	  
		dsend("xm fix_init(0,0,3)"); 
