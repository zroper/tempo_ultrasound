...
declare initfix_punishtime = 2000;	  		 	// Amount of wait time for punishment if initial fixation not maintained.
declare signal_respond_punishT =2000; 		// Wait time if trial failed due to signal-respond.
...
process STIM()
{
	...
	dsend("rf $1-10, $2-10, $1+10, $2+10");		  //75
	dsendf("co $3;");
	dsend("em");
	////////////////////////////////////////////////////////////////////////////////
	...
	if(types[j] == 2)
	{
		...
		if(Stimu[s]==1 || Stimu[s]==2 || Stimu[s]==3)
		{
			...
			if(Stimu[s]==1 || Stimu[s]==2 || Stimu[s]==3)
				...
				if (failed == 0)//but only if didn't leave fix early
				{
				if (Stimu[s]==2 || Stimu[s]==3)//MWL 6/12/06 added this line
				{
				...
				// Enables photodiode on the monitor to detect stop signal presentation.
				//MWL add coment here to look for marker 2 only if 2 or 3
				// if (Stimu[s]==2 || Stimu[s]==3)//MWL 6/12/06 added 2 or 3 so not searching on isnotnogo
				//	{
	...
	if(!failed)
	{
		...
		{			
			if(Stimu[s]==2 || Stimu[s]==3) // and it is a stimulated stop or regular stop trial
				{
				stop_signal_startT = time() + stop[s]+ 12;					    // add 12ms for ASL delay Find out how long to wait...
