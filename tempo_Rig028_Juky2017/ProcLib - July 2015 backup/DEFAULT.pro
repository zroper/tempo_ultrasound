...
/* 			 Color_list[0,r_]		= 35;	// gray
		...
		if (state == run_search_sess)
			...
			LatStruct				= 0; //0 = normal target/dist positions, 1 = only lateral and up/down positions
			//TrainOrt 				= 1;
			...
			TargOrt					= 1; //T/L - 1=UP, 2=INV, 3=LEFT, 4=RIGHT  
			...
			catch_hold_time			= 200;
			Perc_catch				= 10; //percent catch trials
			TargetType				= 1; //1 = L, 2 = T
...
if(monkey == gauss)
		...
		if (state == run_search_sess)
			...
			LatStruct				= 0; //0 = normal target/dist positions, 1 = only lateral and/or up/down positions (lat only vs lat +up/down depends on TgAng set in LOCRAND.pro being 2 vs 4)
			...
			SearchEcc				= 8; //Eccentricity in degrees; use to make fixed eccentricity 
			SingMode				= 1; //0=classic search, 1=singleton present/capture task
			SingCol					= 1; 
			PercSingTrl				= 50; //Percentage of trials where singleton is present, see LOC_RAND.pro for code
			soa_mode				= 0; //fixation response soa; 1=on, 0=off 
			...
			TargetType				= 1; //1 = L, 2 = T
			PlacPres				= 1; //1 = no placeholders,  2 = placeholders
			SetSize					= 1; //SS1 = 1, SS2 = 2, etc. up to set size 12
			// Select Search task and Target/Distractor for Singleton Search
