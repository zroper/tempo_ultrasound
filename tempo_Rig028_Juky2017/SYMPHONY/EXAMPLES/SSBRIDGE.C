/*  .title  SSBRIDGE.C - Example of a bridging node between TEMPO and an external device
;+
; SYNOPSIS
;   SSBRIDGE        (run by ELSIE SSBRIDGE.LC)
;
;
; DESCRIPTION
;   This sample program illustrates how a NODE can receive strings from
;   the TEMPO protocol.
;
;   These strings can be interpreted and actions can be taken based on
;   them.  This example provides a general framework for allowing a
;   TEMPO protocol to communicate information to a Symphony node with strings.
;
;   Of course, it is up to the user to implement any desired action based
;   on the strings sent by the protocol.
;
;   This example program works with the SSBRIDGE.PRO protocol to illustrate
;   how a protocol sends strings to a Symphony NODE.  This program can parse
;   and interpret the strings and communicates with an external device or
;   computer.  It also provides an example of how to use TEMPOW's Symphony
;   server to execute TEMPO commands (ie to set protocol variables).
;
;   To test SSBRIDGE, follow these steps
;
;       1. Start TEMPOW and run in TEMPOW's command window:
;           
;               CD /tempo/prowin
;               CLOAD SSBRIDGE
;               START
;
;           The SSBRIDGE.PRO will wait to get the LCID that we (SSBRIDGE.C) send it.
;       
;       2. Run the Symphony Computation with the SSBRIDGE node:
;          In a Command Window, type:
;
;               cd \tempo\symphony\examples
;               elsie ssbridge.lc
;
;          The SSBRIDGE.C program will set the SSBRIDGE.PRO protocol variables
;          lcidHigh, lcidLow to the LCID of this computation.
;
;          The SSBRIDGE.PRO protocol waits for lcidHigh and lcidLow to be set.
;          Once they are set, it sends one or more strings by executing:
;
;               systemf("SS SEND # '...'\n");
;   
;          This program (SSBRIDGE.C) receives these strings in the tgcbRev()
;          callback function and prints them.
;
;          In a 'real' bridging program, this is the point where you will
;          need to parse the strings sent by the protocol and perform the
;          desired actions (ie communicate with your external device).
;
;       3. To exit, STOP the protocol and type ESC key in ELSIE's window
;
; OUTPUT
;
;   When you follow the above directions, the output you should see for
;   this program looks something like this:
;
;       SSBRIDGE must be run by ELSIE, not from the command line.
;       ssBridge - SSBRIDGE Opened LOG file 'SSBRIDGE.log' on COMPUTER 'HORIZON'
;       Waiting for START ... Press the ESC key to abort.
;       STATE INVITE: Our Node is 'ssBridge' LCID 39021166769944
;       NODE ssBridge: STATE 1000000006 RECEIVED AND IGNORED.
;       NODE ssBridge: STATE 1000000007 RECEIVED AND IGNORED.
;       NODE ssBridge: STATE 1000000008 RECEIVED AND IGNORED.
;       ELSIE assigned this NODE name 'ssBridge' LCID 39021166769944
;       Sending TEMPO command (LCID=237D52C8B718: 'lcidHigh = 0x237D; lcidLow=0x52C8B718;   ' ...
;       Press ESC in ELSIE to terminate the computation.
;       --------------------------------
;       tgcbRecv tempow: test line 1
;       --------------------------------
;       --------------------------------
;       tgcbRecv tempow: test line 2
;       --------------------------------
;       --------------------------------
;       tgcbRecv tempow: test line 3
;       --------------------------------
;
;
; ACKNOWLEDGEMENTS
;   Thanks to Mike Page for his help with this.
;
;
; SEE ALSO
;   ExampleT.C, TEMPO\PROWIN\SSBRIDGE.PRO
;
;
; EDIT HISTORY
...
#define PROGRAM_NAME    "SSBRIDGE"              // Name of this program
#define LOG_FILE        PROGRAM_NAME ".log"     // Name of LOG file
...
printf(PROGRAM_NAME " must be run by ELSIE, not from the command line.\n");
...
if (_access(LOG_FILE, 0) == 0)
    remove(LOG_FILE);                   // Delete previous log file
    
myprintf("%s - " PROGRAM_NAME " Opened LOG file '%s' on COMPUTER '%s'\n",
    av[0],
    LOG_FILE,
    thisComputerName);
...
//------------------------------------------------------------------    
// We've been invited into a computation.
// Let's tell the TEMPO protocol the LCID of this computation
// so it can send us command strings.

sprintf(tempoCommand, "QUIET lcidHigh = 0x%lX; QUIET lcidLow=0x%lX",
    HIDWORD64(myLCID),
    LODWORD64(myLCID));
    
myprintf("Sending TEMPO command (LCID=%I64X: '%s' ...\n", myLCID, tempoCommand);
...
(void) FIOWriteStrToFilename(LOG_FILE, "%02u.%02u.%03u: %s",
    systime.wMinute,
    systime.wSecond,
...
//
// This is the function that is called when the TEMPO protocol sends a
// string to us when it executes
//
//       systemf("SS SEND # '...'\n");
//
// The string sent is in rr->rqst.
