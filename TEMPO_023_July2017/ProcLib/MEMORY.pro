...
process MEMORY()     
	{
	declare hide int run_mem_sess	= 3;
	...
	if (Last_task != run_mem_sess)				// Only do this if we have gone into another task or if this is first run of day.
		{
	...
	spawnwait SETM_TRL(n_targ_pos,				// Select variables for the first mem guided...					
	...
	while (State == run_mem_sess)				// while the user has not yet terminated the mem guided task
		{
		

		spawnwait MEMTRIAL(allowed_fix_time,	// run a trial with variables defined by SETM_TRL.pro
				curr_holdtime,
				curr_soa,			
				max_saccade_time, 		
				max_sacc_duration, 		
